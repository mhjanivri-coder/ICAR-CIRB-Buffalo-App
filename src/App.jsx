...
export default function AnimalDataRecordingApp() {
  ...
  const [productionFridayDraft, setProductionFridayDraft] = useState({ morningMilk: "", eveningMilk: "", totalDailyYield: "" });
  ...
  useEffect(() => {
    ...
    const eveningMilk = existing?.eveningMilk || "";
    const totalDailyYield = morningMilk !== "" || eveningMilk !== "" ? (Number(morningMilk || 0) + Number(eveningMilk || 0)).toFixed(1) : (existing?.totalDailyYield || "");
    setProductionFridayDraft({ morningMilk, eveningMilk, totalDailyYield });
  ...
  function saveProductionFridayEntry() {
    ...
    if (productionFridayDraft.morningMilk === "" && productionFridayDraft.eveningMilk === "") {
      setProductionFridayMessage("Please enter Morning MY or Evening MY before saving.");
      return;
    }
    ...
    setProductionFridayMessage(`Saved Friday milk record for ${targetFriday} in production parity ${selectedParityNo}.`);
  }
            ...
            <Section title="Monthly Herd Strength Report">
              ...
              <div className="space-y-5">
                ...
                <div className="rounded-[24px] border border-slate-100 bg-white/70 px-4 py-4">
                  <div className="text-sm text-slate-600">* Experimental Animals</div>
                  <div className="mt-1 text-xl font-bold text-slate-950">Total Herd = {herdStructureReport.calfTotals.total + herdStructureReport.adultTotals.total}</div>
                </div>
              </div>
            </Section>
                                ...
                                <div className="space-y-3">
                                  <div className="rounded-[24px] border border-emerald-100 bg-white p-4 shadow-sm">
                                    <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
                                      <div>
                                        <div className="text-sm font-semibold uppercase tracking-[0.18em] text-emerald-700">In-situ Friday milk entry</div>
                                        <div className="mt-1 text-sm text-slate-600">Enter this animal’s Friday record directly here, or use Test Day Records for whole-herd WA/HA entry.</div>
                                      </div>
                                      <div className="grid gap-3 md:grid-cols-[170px_130px_130px_120px_auto] md:items-end">
                                        <FridayDatePicker label="Friday date" value={selectedProductionFriday} onChange={setSelectedProductionFriday} />
                                        <TextField label="Mor MY" value={productionFridayDraft.morningMilk || ""} onChange={(v) => updateProductionFridayDraft("morningMilk", v)} />
                                        <TextField label="Eve MY" value={productionFridayDraft.eveningMilk || ""} onChange={(v) => updateProductionFridayDraft("eveningMilk", v)} />
                                        <TextField label="Total MY" value={productionFridayDraft.totalDailyYield || ""} onChange={() => {}} readOnly />
                                        <button className="rounded-xl bg-emerald-600 px-4 py-3 font-semibold text-white shadow-sm hover:bg-emerald-700" onClick={saveProductionFridayEntry}>Save</button>
                                      </div>
                                    </div>
                                    {productionFridayMessage && <div className="mb-3 rounded-2xl bg-emerald-50 px-4 py-2 text-sm text-emerald-800">{productionFridayMessage}</div>}
